...
public:
    ...
    Directory* open(uint32_t inode_no);
    void       close();
    bool       search_entry(const char* name, DirectoryEntry* directory_entry);
    void       create_entry(char* filename, uint32_t inode_no, FileType file_type, DirectoryEntry* directory_entry);
    ...
    DirectoryEntry* read(Directory* directory);
    bool            is_empty(Directory* directory);
    int32_t         remove(Directory* parent_directory, Directory* child_directory);
